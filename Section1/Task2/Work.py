...
def detect_preamble_cross_correlation(signal, preamble, ch):
    ...
    for i in range(len2 - len1):
        ar[i] += abs(np.correlate(preamble, signal[i:i + len1], 'valid'))
...
print("the start_index of received_signal_weak(cross_correlation):")
print(detect_preamble_cross_correlation(received_signal_weak, PSTS_sig, "Weak Signal(cross_correlation)"))
print("the start_index of received_signal_strong(cross_correlation):")
print(detect_preamble_cross_correlation(received_signal_strong, PSTS_sig, "Strong Signal(cross_correlation)"))

# Method 2 auto_correlation
length = 16  # 320是整个OFDM前序的长
print("the start_index of received_signal_weak(auto_correlation):")
print(detect_preamble_auto_correlation(received_signal_weak, length, "Weak Signal(auto_correlation)"))
print("the start_index of received_signal_strong(auto_correlation):")
print(detect_preamble_auto_correlation(received_signal_strong, length, "Strong Signal(auto_correlation)"))
...
# Method 4 sliding
length = 16
print("the start_index of received_signal_weak(sliding):")
print(detect_preamble_sliding(received_signal_weak, length, "Weak Signal(sliding)"))
print("the start_index of received_signal_stong(sliding):")
print(detect_preamble_sliding(received_signal_strong, length, "Strong Signal(sliding)"))
